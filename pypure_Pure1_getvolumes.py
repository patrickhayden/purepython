...
urllib3.disable_warnings()  # Disable warnings (use with caution in production)
...
def main():
    ...
    app_id = "pure1:apikey:zzzzzzzzzzzzzzzz"  # Replace with actual app ID from Pure1
    client = get_client(app_id)

    if client:
        print("Client obtained successfully. Proceeding with API calls.\n")
        try:
            response = client.get_volumes()  # Example: Get volumes
